...
- **Lemma 8.4（正确性）**：n 个 d 位数，按位稳定排序 d 轮，得到整体有序。（归纳：排到第 i 位时，前 i 位已正确有序。）
- **Lemma 8.3**：n 个 b 位、基数 r 的数，需要 ⌈b/r⌉ 轮，每轮 Θ(n + r)，总 **Θ((b/r)·(n+r))**。
...
- **8.2-4**　给定 [0,k] 内 n 个数，回答「区间 [a,b] 内有多少个元素」：计数数组 C 算前缀和后，O(1) 回答 `C[b] − C[a−1]`。
- **8.3-1**　用基数排序演示 `COW,DOG,SEA,RUG,ROW,MOB,BOX,TAB,EAR,TAR,DIG,BIG,TEA,NOW,FOX`：从最后一列字母逐列向前稳定排序。
...
- **8-3 变位词排序**：把每组变位词内部排序作 key，再整体排序——O(nk lg k) 类线性时间。
